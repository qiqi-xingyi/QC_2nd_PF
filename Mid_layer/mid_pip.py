...
# --*-- coding: utf-8 --*--
# Core SS middleware: geometry, H-bond kernels, E_SS energy, objective wrapper.
...
def load_prior(path: str) -> Dict[str, np.ndarray]:
    ...
    Load prior.npz with fields:
      ...
      - beta_pairs: (K,3) rows (p, q, conf), p<q, 0<=conf<=1; may be empty
    """
...
def _torsion(p1: np.ndarray, p2: np.ndarray, p3: np.ndarray, p4: np.ndarray) -> float:
    """Return dihedral angle (degrees) for four 3D points using right-hand rule around (p2-p3)."""
    b1 = p2 - p1
    b2 = p3 - p2
    b3 = p4 - p3
    n1 = np.cross(b1, b2)
    ...
    x = float(np.dot(n1n, n2n))
    m = np.cross(n1n, b2n)
    y = float(np.dot(m, n2n))
...
class BackboneIndices:
    """Indices of backbone atoms within coords[..., atoms, 3]."""
    N: int
    CA: int
    C: int
    O: Optional[int] = None  # O may be missing
...
def compute_dihedrals(coords: np.ndarray, bb: BackboneIndices = DEFAULT_BB) -> Tuple[np.ndarray, np.ndarray]:
    ...
    Returns:
      phi[L], psi[L] in degrees. Endpoints without a well-defined angle are np.nan.
    """
    L = coords.shape[0]
...
class HBKernel:
    ...
    sigma_d: float = 0.25
    theta0: float = 160.0 # degrees, ideal angle (D-H...A)
    sigma_theta: float = 20.0
...
def alpha_hb_scores(
    coords: np.ndarray,
    P_alpha: np.ndarray,
    bb: BackboneIndices = DEFAULT_BB,
    kernel: HBKernel = HBKernel(),
) -> List[Dict[str, float]]:
    """
    Compute alpha-helix i->i+4 hydrogen bond geometry scores.
    Uses O(i)...N(i+4) distance and a pseudo angle from C(i)->O(i) to O(i)->N(i+4).
    Returns list of {'i': i, 'score': s}.
    """
    L = coords.shape[0]
    out: List[Dict[str, float]] = []
    has_O = (bb.O is not None) and (coords.shape[1] > bb.O)
    for i in range(0, L - 4):
        ...
        if has_O:
            ...
            O_i = coords[i, bb.C]
            C_i = coords[i, bb.C]
        N_ip4 = coords[i + 4, bb.N]
        d = float(np.linalg.norm(O_i - N_ip4))

        v1 = O_i - C_i
        v2 = N_ip4 - O_i
        nu = float(np.linalg.norm(v1)); nv = float(np.linalg.norm(v2))
        if nu < 1e-8 or nv < 1e-8:
            theta = 0.0
        else:
            c = float(np.clip(np.dot(v1, v2) / (nu * nv), -1.0, 1.0))
...
def beta_hb_scores(
    coords: np.ndarray,
    beta_pairs: np.ndarray,
    bb: BackboneIndices = DEFAULT_BB,
    kernel: HBKernel = HBKernel(),
) -> List[Dict[str, float]]:
    """
    Compute geometry scores for candidate beta pair (p,q) provided by prior.
    Uses O(p)...N(q) distance + pseudo angle; returns list of {'p','q','score'}.
    """
    out: List[Dict[str, float]] = []
    ...
    for (p, q, conf) in beta_pairs:
        ...
        if has_O:
            ...
            C_p = coords[p, bb.C]
        N_q = coords[q, bb.N]
        d = float(np.linalg.norm(O_p - N_q))
        v1 = O_p - C_p
        v2 = N_q - O_p
        nu = float(np.linalg.norm(v1)); nv = float(np.linalg.norm(v2))
        if nu < 1e-8 or nv < 1e-8:
...
def ss_energy(
    ...
    for i in range(L):
        if np.isnan(phi[i]) or np.isnan(psi[i]):
            continue
        pA, pB, pC = float(P_ss[i, 0]), float(P_ss[i, 1]), float(P_ss[i, 2])
        ...
        E_tor += pA * (params.kappa_phi_alpha * sq(phi[i] - mu_phi_alpha) + params.kappa_psi_alpha * sq(psi[i] - mu_psi_alpha))
        E_tor += pB * (params.kappa_phi_beta  * sq(phi[i] - mu_phi_beta)  + params.kappa_psi_beta  * sq(psi[i] - mu_psi_beta))
        E_tor += pC * (params.kappa_phi_coil  * sq(phi[i] - mu_phi_coil)  + params.kappa_psi_coil  * sq(psi[i] - mu_psi_coil))

    E_alpha = 0.0
    for g in alpha_scores:
        i = g["i"]
        E_alpha += - params.kappa_alpha_hb * float(P_ss[i, 0] * P_ss[i + 4, 0]) * float(g["score"])

    E_beta = 0.0
    for g in beta_scores:
        p, q = int(g["p"]), int(g["q"])
        E_beta += - params.kappa_beta_hb * float(P_ss[p, 1] * P_ss[q, 1]) * float(g["score"])

    return float(E_tor + E_alpha + E_beta)

def ss_energy_with_terms(
    phi: np.ndarray,
    psi: np.ndarray,
    P_ss: np.ndarray,                       # (L,3) [alpha,beta,coil]
    alpha_scores: List[Dict[str, float]],
    ...
    """
    Return (E_SS, terms) where terms includes decomposed components:
      - E_torsion, E_alphaHB, E_betaHB
      - alpha_hb_sum, beta_hb_sum
      - phi_rmse, psi_rmse  (vs per-residue targets if provided, else vs class centers)
      - P_alpha_mean, P_beta_mean, P_coil_mean
    """
    L = len(phi)
    assert P_ss.shape == (L, 3)
    P_alpha, P_beta, P_coil = P_ss[:, 0], P_ss[:, 1], P_ss[:, 2]

    # centers for RMSE
    if use_per_residue_mu is None:
        # weighted coarse centers per residue
        mu_phi = (P_alpha * params.mu_alpha_phi + P_beta * params.mu_beta_phi + P_coil * params.mu_coil_phi) / np.clip(P_alpha + P_beta + P_coil, 1e-8, None)
        mu_psi = (P_alpha * params.mu_alpha_psi + P_beta * params.mu_beta_psi + P_coil * params.mu_coil_psi) / np.clip(P_alpha + P_beta + P_coil, 1e-8, None)
    else:
        mu_phi, mu_psi = use_per_residue_mu
    ...
    for i in range(L):
        if np.isnan(phi[i]) or np.isnan(psi[i]):
            continue
        pA, pB, pC = float(P_alpha[i]), float(P_beta[i]), float(P_coil[i])
        if use_per_residue_mu is None:
            mu_phi_alpha, mu_psi_alpha = params.mu_alpha_phi, params.mu_alpha_psi
            mu_phi_beta,  mu_psi_beta  = params.mu_beta_phi,  params.mu_beta_psi
            mu_phi_coil,  mu_psi_coil  = params.mu_coil_phi,  params.mu_coil_psi
        else:
            mu_phi_alpha = mu_phi_beta = mu_phi_coil = float(mu_phi[i])
            mu_psi_alpha = mu_psi_beta = mu_psi_coil = float(mu_psi[i])
    ...
    # HB rewards and sums
    alpha_hb_sum = float(np.sum([g["score"] for g in alpha_scores])) if alpha_scores else 0.0
    beta_hb_sum = float(np.sum([g["score"] for g in beta_scores])) if beta_scores else 0.0

    E_alpha = 0.0
    for g in alpha_scores:
        i = g["i"]
        if i + 4 < L:
            E_alpha += - params.kappa_alpha_hb * float(P_alpha[i] * P_alpha[i + 4]) * float(g["score"])

    E_beta = 0.0
    for g in beta_scores:
        p, q = int(g["p"]), int(g["q"])
        E_beta += - params.kappa_beta_hb * float(P_beta[p] * P_beta[q]) * float(g["score"])

    valid = ~np.isnan(phi) & ~np.isnan(psi)
    if np.any(valid):
        phi_rmse = float(np.sqrt(np.mean((phi[valid] - mu_phi[valid]) ** 2)))
        psi_rmse = float(np.sqrt(np.mean((psi[valid] - mu_psi[valid]) ** 2)))
    else:
        phi_rmse = psi_rmse = float("nan")

    terms = {
        "E_torsion": float(E_tor),
        "E_alphaHB": float(E_alpha),
        "E_betaHB": float(E_beta),
        "alpha_hb_sum": alpha_hb_sum,
        "beta_hb_sum": beta_hb_sum,
        "phi_rmse": phi_rmse,
        "psi_rmse": psi_rmse,
        "P_alpha_mean": float(np.mean(P_alpha)),
        "P_beta_mean": float(np.mean(P_beta)),
        "P_coil_mean": float(np.mean(P_coil)),
    }
    return float(E_tor + E_alpha + E_beta), terms
...
class Anneal:
    ...
    def value(self, step: int, total_steps: int | None = None) -> float:
        ...
        cutoff = max(1, int(self.warmup_frac * total_steps))
        if step <= cutoff:
            t = step / float(cutoff)
...
class ObjectiveWrapper:
    ...
    ss_params: SSEnergyParams = SSEnergyParams()
    ...
    def objective(self, state: Any, step: int, total_steps: int | None = None) -> float:
        ...
        This is the only function your classical optimizer needs to call (for in-loop).
        """
        ...
        # HB lists computed with default kernel here; you can customize if needed
        alpha_list = alpha_hb_scores(coords, P_alpha=P_ss[:, 0], bb=DEFAULT_BB, kernel=self.hb_kernel)
        beta_list = beta_hb_scores(coords, beta_pairs=self.prior.get("beta_pairs", np.zeros((0, 3), np.float32)),
                                   bb=DEFAULT_BB, kernel=self.hb_kernel)
        mu_tuple = (self.prior["phi_mu"], self.prior["psi_mu"]) if self.use_per_residue_mu else None
...
def build_objective(
    ...
    return ObjectiveWrapper(
        ...
        prior=prior,
        ss_params=ss_params or SSEnergyParams(),
